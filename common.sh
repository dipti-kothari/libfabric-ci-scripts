...
case $job_type in
    PR) 
                compute_node_template_bucket=LibfbaricCI-Compute-Node-Template
                ;;
...
create_resource()
{
    ...
    extra_params=""
    if [[ $job_type == PR ]]; then
        extra_params="ParameterKey=SecurityGroup,ParameterValue=${slave_security_group} ParameterKey=PullRequestId,ParameterValue=${PULL_REQUEST_ID} ParameterKey=PullRequestRef,ParameterValue=${PULL_REQUEST_REF} ParameterKey=TARGET_BRANCH,ParameterValue=${TARGET_BRANCH}"
